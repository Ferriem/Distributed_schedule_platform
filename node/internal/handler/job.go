...
import (
	...
	"github.com/Ferriem/Distributed_schedule_platform/common/pkg/utils/errors"
...
func GetJobs(nodeUUID string) (jobs Jobs, err error) {
	...
	for _, j := range resp.Kvs {
		...
		if e := json.Unmarshal(j.Value, job); e != nil {
			logger.GetLogger().Warn(fmt.Sprintf("unmarshal job[%s] err: %s", string(j.Value), e.Error()))
			continue
		}
		if err := job.Check(); err != nil {
			logger.GetLogger().Warn(fmt.Sprintf("job[%s] is invalid: %s", job, err.Error()))
			continue
		}
		jobs[job.ID] = job
	}
...
func (j *Job) RunWithRecovery() {
	...
	if runErr != nil {
		...
		if err != nil {
			logger.GetLogger().Warn(fmt.Sprintf("Failed to write to job log with jodID:%d nodeUUID:%s error:%s", j.ID, j.RunOn, err.Error()))
		}
		...
		msg := &notify.Message{
			...
			Subject:   fmt.Sprintf("Task[%s] failed", j.Name),
...
func CreateJob(j *Job) cron.FuncJob {
	...
	jobFunc := func() {
		...
		msg := &notify.Message{
			...
			IP:        fmt.Sprintf("%s:%s", node.IP, node.PID),
			Subject:   fmt.Sprintf("Task[%s] failed", j.Name),
			Body:      fmt.Sprintf("job[%d] run on node[%s] execute failed, retry %d times, output:%s, error:%s", j.ID, j.RunOn, j.RetryTimes, output, runErr.Error()),
			To:        to,
			OccurTime: time.Now().Format(utils.TimeFormatSecond),
		}
		go notify.Send(msg)
