...
type JobLog struct {
	...
	IP       string `json:"ip" gorm:"size:32;column:ip"`
...
func (jb *JobLog) Delete() error {
	return dbclient.GetMysqlDB().Table(CronyJobLogTableName).Exec(fmt.Sprintf("delete from %s where id = ?", CronyJobLogTableName), jb.ID).Error
}
