...
type Node struct {
	...
	UUID     string `json:"uuid" gorm:"size:128;column:uuid;not null;idex:idx_node_uuid;"`
...
func (n *Node) Delete() error {
	return dbclient.GetMysqlDB().Table(CronyNodeTableName).Exec(fmt.Sprintf("delete from %s where uuid = ?", CronyNodeTableName), n.UUID).Error
}
